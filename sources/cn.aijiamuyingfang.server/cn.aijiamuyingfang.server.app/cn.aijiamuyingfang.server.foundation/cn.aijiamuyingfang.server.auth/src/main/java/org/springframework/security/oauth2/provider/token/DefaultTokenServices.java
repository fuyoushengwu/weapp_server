...
/**
 * Base implementation for token services using random UUID values for the access token and refresh
 * token values. The main extension point for customizations is the {@link TokenEnhancer} which will
 * be called after the access and refresh tokens have been generated but before they are stored.
 * <p>
 * Persistence is delegated to a {@code TokenStore} implementation and customization of the access
 * token to a {@link TokenEnhancer}.
 * 
 * @author Ryan Heaton
 * @author Luke Taylor
 * @author Dave Syer
...
public class DefaultTokenServices
   ...
   *           If the scope requested is invalid or wider than the original scope.
   */
   ...
   * tokens. If a client details service is set the validity period will be read from he client,
