...
public class ShopOrderControllerClient {
   ...
   * @throws IOException
   */
  public void updatePreOrder(String token, GoodRequest request, boolean async) throws IOException {
    if (async) {
      shoporderControllerApi.updatePreOrder(token, request).enqueue(Empty_Callback);
      return;
    }
